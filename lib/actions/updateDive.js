...
export const updateDive = async (formData, id) => {
  ...
  try {
    ...
    // Get dive from before update to remove cloudinary image if a new one is uploaded and check userid
    const oldDiveData = await Dive.findById(id);

    if (oldDiveData.userId?.toString() !== session.user.id) {
      return {
        error: "Unauthorized",
      };
    }
    ...
    if (formData.get("image")?.size > 0) {
      ...
      if (oldDiveData.image) {
        const imageId = oldDiveData.image.split('/').pop().replace(/\.(.*)/, '');
